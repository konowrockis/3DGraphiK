...
*/
__global__ void KernMapToBoolean(int n, int *bools, const Triangle *idata) {
	int index = threadIdx.x + (blockIdx.x * blockDim.x);
	bools[index] = !!idata[index].Visible;
}
...
*/
__global__ void KernScatter(int n, Triangle *odata, const Triangle *idata, const int *bools, const int *indices) {
